...
def extract_dilution(name):
    """Detects 10x, 10 x, or 100 x from sample names."""
...
def parse_manual(text):
    """Splits text by 'Sample' headers and merges rows by Sample Name."""
    blocks = re.split(r'(?mi)^Sample', text)
    master_data = {}
    for block in blocks:
        if not block.strip(): continue
        # Use regex to handle tabs or multiple spaces
        lines = [re.split(r'\t|\s{2,}', line.strip()) for line in block.strip().split('\n') if line.strip()]
        ...
        for row in lines[1:]:
            ...
            if not s_name or any(x in s_name.lower() for x in ['mg/l', 'unit', 'avg', 'sd']): continue
...
raw_pasted_text = st.text_area("Paste your ICP blocks here (Sample names must match to auto-concatenate):", height=250)

if raw_pasted_text:
    df_raw = parse_manual(raw_pasted_text)
    
    if df_raw is not None:
        # Clean numeric data (remove < and >)
        for col in df_raw.columns:
            if col != 'Sample':
                df_raw[col] = pd.to_numeric(df_raw[col].astype(str).str.replace('<','').str.replace('>',''), errors='coerce')

        # Filter out Control Standards
        df_filtered = df_raw[~df_raw['Sample'].str.contains('control', case=False, na=False)].copy()
        ...
        detected = sorted(list(set([
            ...
            for c in df_filtered.columns if re.match(rf"^{e}([^a-zA-Z]|$)", c.strip())
        ])))
        
        st.subheader("3. Select Reporting Mode")
        modes = {}
        if detected:
            cols = st.columns(min(len(detected), 10))
            for i, e in enumerate(detected):
                modes[e] = cols[i % 10].radio(f"**{e}**", ["Elem", "Oxide"], key=f"m_{e}")

        # --- 4. CALCULATIONS ---
        results = []
        for _, row in df_filtered.iterrows():
            sn = row['Sample']
            pm = p_map.get(sn, {'Vol (mL)': 500.0, 'Dilution': 1.0, 'Mass (g)': 0.5, 'Moisture (%)': 0, 'LOI (%)': 0})
            
            res, total = {"Sample": sn}, 0.0
            
            for elem in detected:
                # Find all columns for this element (handle multiple wavelengths)
                m_cols = [c for c in df_filtered.columns if re.match(rf"^{elem}([^a-zA-Z]|$)", c.strip())]
                val = pd.to_numeric(row[m_cols], errors='coerce').mean()
                
                if not np.isnan(val):
                    # Calculation: (mg/L * Vol_L * Dilution) / (Mass_g * 1000) * 100
                    factor = (pm['Vol (mL)']/1000 * pm['Dilution']) / (pm['Mass (g)'] * 1000)
                    perc = val * factor * 100
                    
                    if modes[elem] == "Oxide":
                        oxide_name, conv_factor = element_to_oxide[elem]
                        perc_oxide = perc * conv_factor
                        res[f"{oxide_name} (%)"] = round(perc_oxide, 4)
                        total += perc_oxide
                    else:
        ...
        if results:
            ...
            st.download_button("Download Results CSV", csv, "icp_output.csv", "text/csv")
        else:
            st.info("Paste data above to see results.")
